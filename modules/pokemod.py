    ...
    try:
      async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://pokeapi.co/api/v2/pokemon/{pokemon_name}") as resp:
          if resp.status == 404:
            await interaction.response.send_message(
                f"Pokémon '{pokemon_name}' not found!", ephemeral=True)
            return
...
class PokemonNavigationButton(discord.ui.Button):
  ...
  async def callback(self, interaction: discord.Interaction):
    ...
    try:
      ...
      loading_embed.set_thumbnail(url=view.pokemon_data['sprites']['other']
      ...
      async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://pokeapi.co/api/v2/pokemon/{pokemon_id}") as resp:
          if resp.status == 404:
...
class PokemonInfoView(discord.ui.View):
  ...
  async def create_loading_embed(self) -> discord.Embed:
    ...
    if self.pokemon_data and self.pokemon_data['sprites']['other'][
        'official-artwork']['front_default']:
      loading_embed.set_thumbnail(url=self.pokemon_data['sprites']['other']
                                  ['official-artwork']['front_default'])

    loading_embed.add_field(name="Status", value="⌛ Loading...", inline=False)
  ...
  async def create_stats_embed(self) -> discord.Embed:
    ...
    stats_text = ""
    max_stat = max(stat['base_stat'] for stat in self.pokemon_data['stats'])
  ...
  async def create_moves_embed(self) -> discord.Embed:
    pokemon_color = await self.get_pokemon_color()
    embed = discord.Embed(title=f"{self.pokemon_data['name'].title()} - Moves",
                          color=pokemon_color)
    embed.set_thumbnail(url=self.pokemon_data['sprites']['other']
                        ['official-artwork']['front_default'])
    ...
    for move in self.pokemon_data['moves']:
      for detail in move['version_group_details']:
        if detail['move_learn_method']['name'] == 'level-up':
          ...
          tm_moves.append(move['move']['name'].replace('-', ' ').title())
        elif detail['move_learn_method']['name'] == 'egg':
          egg_moves.append(move['move']['name'].replace('-', ' ').title())
        break
    ...
    if tm_moves:
      embed.add_field(name="TM/TR Moves",
                      value=", ".join(tm_moves[:15]) +
                      "..." if len(tm_moves) > 15 else ", ".join(tm_moves),
                      inline=False)

    if egg_moves:
      embed.add_field(name="Egg Moves",
  ...
  async def create_main_embed(self) -> discord.Embed:
    ...
    async with aiohttp.ClientSession() as session:
      ...
      async with session.get(species_data['evolution_chain']['url']) as resp:
    ...
    def get_evolution_chain(chain, target_name, prev_chain=None):
      ...
      if current_name == target_name:
        ...
        for evo in chain['evolves_to']:
          ...
          evo_details.append(get_evo_details(evo['evolution_details']))
      ...
      if next_evos:
        ...
        if any(evo_details):
          embed.add_field(name="How to Evolve",
                          value="\n".join(detail for detail in evo_details
                                          if detail),
                          inline=False)
    ...
    resistances = [
        f"-# {t} (x{multiplier})"
        for t, multiplier in type_effectiveness.items() if 0 < multiplier < 1
    ]
    if resistances:
    ...
    async with aiohttp.ClientSession() as session:
      for type_name in pokemon_types:
        async with session.get(
          ...
          for relation in type_data['damage_relations']['double_damage_from']:
...
class GiveUpButton(discord.ui.Button):
  ...
  async def callback(self, interaction: discord.Interaction):
    ...
    view.add_item(
        SeePokedexButton(self.view.pokemon_data, self.view.original_author))
...
class PokemonGuessView(discord.ui.View):
  ...
  def __init__(self,
    ...
    self.pokemon_image = pokemon_data['sprites']['other']['official-artwork'][
        'front_default']
    self.attempts = 3
    self.original_author = original_author
  ...
  async def stop_game(self,
    ...
    if gave_up:
      ...
      view = View()
      # Pass required data to the SeePokedexButton
      view.add_item(SeePokedexButton(self.pokemon_data, self.original_author))
      await interaction.response.edit_message(
          embed=embed,
          attachments=[discord.File(actual_image, filename="pokemon.png")],
          view=view)
