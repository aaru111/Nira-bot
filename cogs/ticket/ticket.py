...
from .utils.ticket_manager import TicketManager as DataManager
...
class send_transcript_dropdown(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        channel = self.interaction.guild.get_channel(
...
class send_transcript_dropdown_view(discord.ui.View):
    ...
    def __init__(
        ...
        self.add_item(
            send_transcript_dropdown(bot=self.bot,
...
class channel_dropdown(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        channel = self.interaction.guild.get_channel(
            int(interaction.data["values"][0]))
        panel = await channel.send(embed=discord.Embed(
            title=f"{panel_data['panel_title']}",
            description=f"{panel_data['panel_description']}",
            colour=discord.Colour.blurple(),
        ))
        ...
        if self.limitPerUser.value.isnumeric():
            ...
            await interaction.edit_original_response(embed=discord.Embed(
                ...
                (f"\n\nCurrent Panel Moderators: {','.join(panel_moderators)}"
                 if panel_moderators else ""),
                colour=discord.Colour.blurple(),
            ))
            await DataManager.edit_panel_data(message.id, "panel_title",
                                              self.panelTitle.value)
            await DataManager.edit_panel_data(message.id, "panel_description",
                                              self.panelDescription.value)
            await DataManager.edit_panel_data(message.id, "limit_per_user",
...
class edit_panel_edit_modal(discord.ui.Modal, title="Edit Panel"):
    ...
    def __init__(self, bot, interaction, panel_id, paneledit, title: str,
...
class create_ticket_modal(discord.ui.Modal, title="Create a Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Your ticket has been created in {ticket.mention}",
            ephemeral=True)
        ticket_moderators = [
            f"<@&{role_id}>" for role_id in panel_data["panel_moderators"]
        ]
        message = await ticket.send(
            content=
            f"{','.join(ticket_moderators)}, {interaction.user.mention}",
            embed=discord.Embed(
        ...
        await DataManager.create_ticket(self.panel_id, ticket.id,
                                        interaction.user.id)
        await message.pin()
        ...
        if panel_data == None:
            return await interaction.response.send_message(
                embed=discord.Embed(
                    title="Error",
                    description=
                    "This panel has been deleted from the database. Please create a new panel",
                    colour=discord.Colour.red(),
                ),
                ephemeral=True,
            )
        ...
        if i >= panel_data["limit_per_user"]:
            return await interaction.response.send_message(
                embed=discord.Embed(
                    ...
                    description=
                    "You already have the maximum amount of tickets open, please close the other tickets before creating a new one",
                    colour=discord.Colour.red(),
                ),
                ephemeral=True,
            )
...
class panel_creation_views(discord.ui.View):
    ...
    async def add_moderators(self, interaction: discord.Interaction,
                             button: discord.ui.Button):
    ...
    async def submit_panel(self, interaction: discord.Interaction,
                           button: discord.ui.Button):
        if interaction.user.id != self.executor_id:
            return await interaction.response.send_message(
                embed=discord.Embed(
        ...
        if panel_data["panel_moderators"]:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    ...
                    description=
                    "You need to set the panel moderators before submitting the panel",
                    colour=discord.Colour.red(),
                ),
                ephemeral=True,
            )
...
class panel_edit_views(discord.ui.View):
    ...
    async def submit_panel(self, interaction: discord.Interaction,
        ...
        channel = self.bot.get_channel(interaction.channel.id)
        message = await channel.fetch_message(self.panel_id)
        await DataManager.edit_panel_data(self.panel_id, "panel_title",
                                          self.panel_title)
        await DataManager.edit_panel_data(
            self.panel_id,
            "panel_description",
        ...
        await DataManager.edit_panel_data(self.panel_id, "panel_moderators",
...
class ticket(commands.GroupCog):
    ...
    @app_commands.command(
        name="create_panel",
        description="Create a ticket panel in the current category")
    @app_commands.guild_only()
    @app_commands.checks.has_permissions(manage_channels=True)
    @app_commands.checks.cooldown(1, 30, key=lambda i: (i.guild.id))
    async def ticket(self, interaction: discord.Interaction, panel_title: str,
    ...
    async def close_ticket(self, interaction: discord.Interaction):
        ...
        if any(ticket["ticket_id"] == str(interaction.channel.id)
               for ticket in all_tickets):
            panel_id = await DataManager.get_panel_id_by_ticket_id(
                interaction.channel.id)
            ticket = await DataManager.get_ticket_data(panel_id,
                                                       interaction.channel.id)
            if ticket["closed"]:
                return await interaction.response.send_message(
                    embed=discord.Embed(
                ...
                panel_id = await DataManager.get_panel_id_by_ticket_id(
                    interaction.channel.id)
                await DataManager.close_ticket(panel_id,
                                               interaction.channel.id)
            ...
            await interaction.edit_original_response(
                embed=discord.Embed(
                    ...
                    (f"\n\nCurrent Panel Moderators: {','.join([f'<@&{role_id}>' for role_id in panel['panel_moderators']])}"
                     if panel["panel_moderators"] else ""),
                    colour=discord.Colour.blurple(),
                ),
