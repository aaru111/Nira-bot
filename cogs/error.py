...
class Errors(commands.Cog):
    ...
    async def send_error_embed(self, ctx: commands.Context, title: str, description: str) -> None:
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError) -> None:
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            similar_commands = get_close_matches(
                error.args[0].split()[0], [cmd.name for cmd in self.bot.commands], n=3, cutoff=0.6)
            if similar_commands:
            ...
            await self.send_error_embed(ctx, "Not Owner", "Only the bot owner can use this command.")

        elif isinstance(error, commands.NoPrivateMessage):
            await self.send_error_embed(ctx, "No Private Message", "This command cannot be used in private messages.")

        elif isinstance(error, commands.BadArgument):
            await self.send_error_embed(ctx, "Bad Argument", str(error))

        elif isinstance(error, commands.CheckFailure):
            await self.send_error_embed(ctx, "Check Failure", "You do not have permission to execute this command.")

        elif isinstance(error, commands.DisabledCommand):
            await self.send_error_embed(ctx, "Disabled Command", "This command is currently disabled.")

        elif isinstance(error, commands.UserInputError):
            await self.send_error_embed(ctx, "User Input Error", str(error))

        elif isinstance(error, commands.InvalidEndOfQuotedStringError):
            await self.send_error_embed(ctx, "Invalid End of Quoted String", str(error))
            ...
            # Notify developers
            dev_channel: Optional[Union[discord.TextChannel, discord.Thread, discord.VoiceChannel]] = self.bot.get_channel(971651546939031562)  # Replace with your developer channel ID
            if dev_channel:
        ...
        logger.error(f"Guild: {ctx.guild} (ID: {ctx.guild.id})" if ctx.guild else "Guild: None")
